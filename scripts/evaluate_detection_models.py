...
NON_EVALUATED_CLASSES = {"stop_line"}
...
class DetectionEvalConfig:
    ...
    traffic_sign_gt_mode: str = "level_bbs"
    traffic_sign_actor_fallback: bool = True
    traffic_sign_min_bbox_dim_px: int = 4
    traffic_sign_min_bbox_area_px: int = 40
    traffic_sign_min_visible_area_ratio: float = 0.20
    traffic_sign_max_bbox_aspect_ratio: float = 10.0
    allow_rgb_only_if_depth_missing: bool = True
    max_sensor_misses: int = 30
...
def parse_run_config(config_path: Path) -> RunConfig:
    ...
    for item in test_detection.get("eval_classes", EVAL_CLASS_ORDER):
        ...
        if class_name in NON_EVALUATED_CLASSES:
            continue
        if class_name not in eval_classes:
            eval_classes.append(class_name)
    if not eval_classes:
    ...
    eval_cfg = DetectionEvalConfig(
        ...
        traffic_sign_gt_mode=str(test_detection.get("traffic_sign_gt_mode", "level_bbs")).strip().lower(),
        traffic_sign_actor_fallback=to_bool(test_detection.get("traffic_sign_actor_fallback", True), True),
        traffic_sign_min_bbox_dim_px=int(test_detection.get("traffic_sign_min_bbox_dim_px", 4)),
        traffic_sign_min_bbox_area_px=int(test_detection.get("traffic_sign_min_bbox_area_px", 40)),
        traffic_sign_min_visible_area_ratio=float(test_detection.get("traffic_sign_min_visible_area_ratio", 0.20)),
        traffic_sign_max_bbox_aspect_ratio=float(test_detection.get("traffic_sign_max_bbox_aspect_ratio", 10.0)),
        allow_rgb_only_if_depth_missing=to_bool(
            test_detection.get("allow_rgb_only_if_depth_missing", True),
            True,
        ),
        max_sensor_misses=max(1, int(test_detection.get("max_sensor_misses", 30))),
        ...
        *,
        ...
        with self._condition:
            while time.time() < deadline:
                ...
                needs_depth = self.require_depth and not bool(allow_missing_depth)
                if rgb is not None and (not needs_depth or depth is not None):
...
def world_location_to_camera_point(world_location: Any, world_to_camera: Any) -> Optional[Any]:
    if np is None:
        return None
...
def project_camera_point_to_image(point_camera: Any, intrinsics: Any) -> Optional[Tuple[float, float, float]]:
    if np is None:
        return None
...
def project_world_location_to_image(
    ...
    point_camera = world_location_to_camera_point(world_location, world_to_camera)
    if point_camera is None:
        return None
    return project_camera_point_to_image(point_camera, intrinsics)


def clip_camera_segment_to_near_plane(
    start: Any,
    end: Any,
    near_depth: float,
    ...
    if np is None:
        return None
    start_depth = float(start[2])
    end_depth = float(end[2])
    near = max(1e-3, float(near_depth))
    start_visible = start_depth >= near
    end_visible = end_depth >= near
    if start_visible and end_visible:
        return start, end
    if not start_visible and not end_visible:
        return None

    denom = end_depth - start_depth
    if abs(denom) <= 1e-9:
        return None
    t = (near - start_depth) / denom
...
def project_bbox_vertices_to_image_points(
    ...
    for vertex in vertices:
        point_camera = world_location_to_camera_point(vertex, world_to_camera)
        if point_camera is not None:
            camera_points.append(point_camera)

    projected_points: List[Tuple[float, float, float]] = []
    for point in camera_points:
        projected = project_camera_point_to_image(point, intrinsics)
        if projected is not None:
            projected_points.append(projected)

    if len(camera_points) >= 8:
        clip_pairs = (
            (start_idx, end_idx)
            for start_idx in range(len(camera_points))
            for end_idx in range(start_idx + 1, len(camera_points))
        )
        for start_idx, end_idx in clip_pairs:
            if start_idx >= len(camera_points) or end_idx >= len(camera_points):
                continue
            clipped = clip_camera_segment_to_near_plane(
                camera_points[start_idx],
                camera_points[end_idx],
                near_depth,
            )
            if clipped is None:
                continue
...
def project_vertices_to_box(
    ...
    projected_points = project_bbox_vertices_to_image_points(
        vertices=vertices,
...
def traffic_sign_projection_filters(eval_cfg: DetectionEvalConfig) -> GroundTruthFilterConfig:
    base = eval_cfg.gt_filters
    return GroundTruthFilterConfig(
        max_distance_m=base.max_distance_m,
        min_bbox_dim_px=max(1, int(eval_cfg.traffic_sign_min_bbox_dim_px)),
        min_bbox_area_px=max(1, int(eval_cfg.traffic_sign_min_bbox_area_px)),
        min_visible_area_ratio=float(eval_cfg.traffic_sign_min_visible_area_ratio),
        min_depth_visible_ratio=base.min_depth_visible_ratio,
        max_bbox_aspect_ratio=max(1.0, float(eval_cfg.traffic_sign_max_bbox_aspect_ratio)),
    )


def get_city_object_label(*names: str) -> Any:
    if carla is None:
        return None
    labels = getattr(carla, "CityObjectLabel", None)
    if labels is None:
        return None
    for name in names:
...
def is_traffic_sign_actor(actor: Any) -> bool:
    type_id = str(getattr(actor, "type_id", "")).lower()
    if not type_id.startswith("traffic."):
        return False
    ...
    return any(token in type_id for token in sign_tokens)
...
def generate_ground_truth(
    ...
    traffic_sign_records = generate_traffic_sign_ground_truth(
        world=world,
        actors=actors,
        ego_location=ego_location,
        frame_id=frame_id,
    ...
    records.extend(record for record in traffic_sign_records if record.class_name in eval_class_set)
    return records
...
def generate_traffic_light_ground_truth(
    ...
    if mode in {"level_bbs", "level_bbs_then_actor", "both"} and carla is not None:
        traffic_light_label = get_city_object_label("TrafficLight", "TrafficLights")
        try:
            level_bbs = list(world.get_level_bbs(traffic_light_label)) if traffic_light_label is not None else []
        except Exception:
            level_bbs = []
...
def generate_traffic_sign_ground_truth(
    world: Any,
    actors: Any,
    ego_location: Any,
    frame_id: int,
    ...
    image_w, image_h = image_size
    records: List[DetectionRecord] = []
    sign_filters = traffic_sign_projection_filters(eval_cfg)

    mode = str(eval_cfg.traffic_sign_gt_mode or "level_bbs").lower()
    used_level_bbs = False
    if mode in {"level_bbs", "level_bbs_then_actor", "both"}:
        sign_label = get_city_object_label("TrafficSigns", "TrafficSign")
        level_bbs = []
        if sign_label is not None:
            try:
                level_bbs = list(world.get_level_bbs(sign_label))
            except Exception:
                level_bbs = []
        for index, level_bbox in enumerate(level_bbs):
            if ego_location is not None:
            ...
            records.append(
                DetectionRecord(
                    ...
                    bbox=projected.bbox,
                    confidence=1.0,
                    object_id=2_000_000 + index,
                )
            )
            used_level_bbs = True

    should_use_actor_bbox = mode in {"actor_bbox", "both"} or (
        eval_cfg.traffic_sign_actor_fallback and not used_level_bbs
    )
    if not should_use_actor_bbox:
        return records

    try:
        traffic_actors = list(actors.filter("traffic.*"))
    except Exception:
        traffic_actors = list(actors) if isinstance(actors, Iterable) else []

    for actor in traffic_actors:
        if not is_traffic_sign_actor(actor):
            continue
        if ego_location is not None:
            try:
                if ego_location.distance(actor.get_location()) > eval_cfg.gt_filters.max_distance_m:
...
def detect_csv_delimiter(path: Path) -> str:
    try:
        first_line = path.read_text(encoding=CSV_ENCODING, errors="ignore").splitlines()[0]
    except Exception:
        return CSV_DELIMITER
    return ";" if first_line.count(";") > first_line.count(",") else ","


def migrate_csv_delimiter_if_needed(path: Path) -> None:
    if not path.exists():
        return
    old_delimiter = detect_csv_delimiter(path)
    if old_delimiter == CSV_DELIMITER:
        return
    try:
        with path.open("r", newline="", encoding=CSV_ENCODING) as src_file:
            rows = list(csv.reader(src_file, delimiter=old_delimiter))
    except Exception:
        return
    with path.open("w", newline="", encoding=CSV_ENCODING) as dst_file:
        writer = csv.writer(dst_file, delimiter=CSV_DELIMITER, lineterminator="\n")
...
def write_model_metrics_outputs(model_dir: Path, model_name: str, metrics: Dict[str, Any], eval_classes: Sequence[str]) -> None:
    ...
    with csv_path.open("w", newline="", encoding=CSV_ENCODING) as csv_file:
        writer = make_csv_dict_writer(csv_file, SUMMARY_FIELDS)
        writer.writeheader()
        for row in rows:
            writer.writerow(row)
...
def write_comparison_outputs(
    ...
    with comparison_csv.open("w", newline="", encoding=CSV_ENCODING) as csv_file:
        fieldnames = ["metric"] + model_names + ["best_model"]
        writer = make_csv_dict_writer(csv_file, fieldnames)
        writer.writeheader()
    ...
    with per_class_csv.open("w", newline="", encoding=CSV_ENCODING) as csv_file:
        writer = make_csv_dict_writer(csv_file, per_class_fields)
        writer.writeheader()
        for class_name in eval_classes:
            for metric_name in [
                "gt_detections",
...
def append_all_runs_summary(
    ...
    migrate_csv_delimiter_if_needed(summary_path)
    needs_header = not summary_path.exists()
    if not needs_header:
        try:
            with summary_path.open("r", newline="", encoding=CSV_ENCODING) as check_file:
                reader = csv.reader(check_file, delimiter=CSV_DELIMITER)
    ...
    with summary_path.open("a", newline="", encoding=CSV_ENCODING) as csv_file:
        writer = make_csv_dict_writer(csv_file, fieldnames, extrasaction="ignore")
        if needs_header:
            writer.writeheader()
...
def write_run_metadata(
    ...
    metadata = {
        ...
        "test_detection": {
            ...
            "traffic_sign_gt_mode": run_config.eval_cfg.traffic_sign_gt_mode,
            "traffic_sign_actor_fallback": run_config.eval_cfg.traffic_sign_actor_fallback,
            "allow_rgb_only_if_depth_missing": run_config.eval_cfg.allow_rgb_only_if_depth_missing,
            "max_sensor_misses": run_config.eval_cfg.max_sensor_misses,
...
def load_detectors(
    ...
    status_by_model: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    ...
    for model_cfg in eval_cfg.models:
        if not model_cfg.path.exists():
            ...
            if status_by_model is not None:
                status_by_model[model_cfg.name] = {"status": "missing", "path": str(model_cfg.path)}
            if strict_models:
        ...
        try:
            ...
            if status_by_model is not None:
                status_by_model[model_cfg.name] = {
                    "status": "failed",
            ...
            if strict_models:
                raise
            logging.warning("Detector load failed for %s: %s", model_cfg.name, exc)
            continue
        detectors[model_cfg.name] = detector
        if status_by_model is not None:
            status_by_model[model_cfg.name] = {"status": "ok", "path": str(model_cfg.path)}
...
def write_all_outputs(
    ...
    progress_path = out_dir / "output_write_progress.log"

    def log_progress(message: str) -> None:
        out_dir.mkdir(parents=True, exist_ok=True)
        with progress_path.open("a", encoding="utf-8") as progress_file:
            progress_file.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} {message}\n")

    log_progress("start write_all_outputs")
    write_mot_txt(out_dir / "ground_truth.txt", ground_truth_by_frame, is_gt=True)
    log_progress("wrote ground_truth.txt")
    ...
    for model_cfg in eval_cfg.models:
        ...
        log_progress(f"start model {model_name}")
        write_mot_txt(model_dir / "predictions.txt", model_predictions, is_gt=False)
        log_progress(f"wrote predictions for {model_name}")
    ...
    log_progress("wrote run_metadata.json")
    ...
    log_progress("finished write_all_outputs")
...
def make_synthetic_eval_data(
    ...
    gt = {
        ...
        3: [
            ...
            DetectionRecord(3, "traffic_sign", (420, 70, 22, 26), 1.0, 302),
        ],
    }
    predictions_by_model: Dict[str, Dict[int, List[DetectionRecord]]] = {}
    ...
    for index, model_cfg in enumerate(run_config.eval_cfg.models):
        ...
        predictions_by_model[model_cfg.name] = {
            ...
            3: [
                ...
                DetectionRecord(3, "traffic_sign", (420 + offset, 70, 22, 26), 0.86, 7),
            ],
        }
        if index == 0:
...
def run_full_evaluation(run_config: RunConfig, skip_warmup: bool) -> Path:
    ...
    if missing_models:
        ...
        logging.warning("Missing configured model files; those models will receive zero predictions: %s", missing_text)
    ...
    detector_status: Dict[str, Any] = {
        model.name: {"status": "pending", "path": str(model.path)} for model in eval_cfg.models
    }
    frames_processed = 0
    sensor_misses = 0
    rgb_only_mode = False
    rgb_only_warning_logged = False
    partial_error: Optional[str] = None
    output_write_error: Optional[Exception] = None
    outputs_written = False
    detectors: Dict[str, Any] = {}

    def finalize_outputs_before_carla_cleanup() -> None:
        nonlocal outputs_written
        if outputs_written:
            return
        if frames_processed <= 0:
            error_text = partial_error or "No frames were processed."
            write_all_outputs(
                out_dir=out_dir,
                run_config=run_config,
                ground_truth_by_frame=ground_truth_by_frame,
                ...
                dry_run=False,
                extra_metadata={
                    "status": "failed_no_frames",
                    "partial_error": error_text,
                    "detector_status": detector_status,
                    "sensor_misses": int(sensor_misses),
                },
            )
            (out_dir / "evaluation_error.txt").write_text(error_text + "\n", encoding="utf-8")
            outputs_written = True
            logging.info("Evaluation diagnostics written to: %s", out_dir)
            return
        ...
        write_all_outputs(
            ...
            extra_metadata={
                ...
                "detector_status": detector_status,
                "rgb_only_gt_projection": bool(rgb_only_mode),
            }
            if partial_error or detector_status or rgb_only_mode
            else None,
        )
        outputs_written = True
    ...
    try:
        ...
        detectors = load_detectors(
            ...
            status_by_model=detector_status,
        )
        if not skip_warmup:
            for model_name, detector in list(detectors.items()):
                warmup = getattr(detector, "warmup", None)
                if callable(warmup):
                    try:
                        ...
                        detector_status[model_name] = {
                            "status": "failed_warmup",
                            "error": str(exc),
                        }
                        logging.warning("Detector warmup failed for %s; skipping model: %s", model_name, exc)
                        detectors.pop(model_name, None)
        ...
        while eval_cfg.max_frames <= 0 or frames_processed < eval_cfg.max_frames:
            ...
            frame_bgr, depth_map_m = frame_buffer.wait_for_frame(
                snapshot_frame,
                wait_timeout,
                allow_missing_depth=rgb_only_mode,
            )
            if frame_bgr is None and eval_cfg.allow_rgb_only_if_depth_missing:
                frame_bgr, depth_map_m = frame_buffer.wait_for_frame(
                    snapshot_frame,
                    0.1,
                ...
                if frame_bgr is not None:
                    rgb_only_mode = True
                    if not rgb_only_warning_logged:
                        logging.warning(
                            "Depth frames are missing or delayed; continuing with RGB-only GT projection."
                        )
                        rgb_only_warning_logged = True
            if frame_bgr is None:
                sensor_misses += 1
                logging.warning("Skipping CARLA frame %d because RGB frame did not arrive.", snapshot_frame)
                driver.run_step()
                if sensor_misses >= int(eval_cfg.max_sensor_misses):
                    raise RuntimeError(
                        "No usable RGB frames arrived after "
                        f"{sensor_misses} consecutive CARLA ticks. "
                        "Check camera spawning, synchronous mode, and CARLA rendering."
                    )
                continue
            ...
            for model_name, detector in list(detectors.items()):
                ...
                try:
                    ...
                    predictions_by_model[model_name][eval_frame_id] = []
                    detectors.pop(model_name, None)
                    logging.warning("Detector inference failed for %s; skipping model: %s", model_name, exc)
        ...
        try:
            finalize_outputs_before_carla_cleanup()
        except Exception as exc:
            output_write_error = exc
            logging.error("Failed to write evaluation outputs before CARLA cleanup: %s", exc, exc_info=True)
            try:
                out_dir.mkdir(parents=True, exist_ok=True)
                (out_dir / "output_write_error.txt").write_text(str(exc) + "\n", encoding="utf-8")
            except Exception:
                pass
        finally:
            destroy_sensor(rgb_camera)
            destroy_sensor(depth_camera)
            if driver is not None:
                driver.cleanup()
            manager.cleanup()

    global _DETECTOR_KEEPALIVE
    _DETECTOR_KEEPALIVE = list(detectors.values())

    if frames_processed <= 0:
        raise RuntimeError(f"No frames were processed; diagnostics written to: {out_dir}")
    if output_write_error is not None:
        raise RuntimeError(f"Evaluation finished but output writing failed: {output_write_error}")
    if partial_error:
...
def main() -> int:
    ...
    if comparison_txt.exists():
        print(comparison_txt.read_text(encoding="utf-8").strip())
    sys.stdout.flush()
    sys.stderr.flush()
    if not args.dry_run:
        os._exit(0)
