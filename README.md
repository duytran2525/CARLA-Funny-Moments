...
*   [traffic_supervisor.py](core_control/traffic_supervisor.py): Bộ giám sát luật lệ giao thông phản động (YOLO). Phát hiện xe chắn phía trước, đèn giao thông đỏ để can thiệp phanh tức thời.
...
*   [yolo_detector.py](core_perception/yolo_detector.py): Bộ tích hợp nhận diện vật thể YOLOv8 và tracking BoT-SORT/ByteTrack.
...
python run_agents.py --agent cil_yolo --cil-model-path models/waypoint_predictor_csv.pth --yolo-model-path models/yolo26m_best.engine --enable-gtnet --gtnet-model-path models/gtnet_full.pt
```
...
*   **Mô hình Nhận diện Đèn đỏ & Vật cản (YOLO):** Việc sử dụng TensorRT engine `models/yolo26m_best.engine` là bắt buộc trong môi trường thời gian thực. Nếu sử dụng tệp `.pt` thông thường, FPS nhận dạng sẽ giảm từ ~150 FPS xuống < 20 FPS, gây trễ nghiêm trọng cho bộ phanh khẩn cấp phản động.
...
*   **Cơ chế Brake Fusion:** Kết hợp phanh phản động (YOLO) và phanh chủ động (GTNet) qua hàm lấy cực đại. Cơ chế này đảm bảo an toàn cao nhất bằng cách ưu tiên lệnh phanh lớn nhất bất kể nguồn gốc nguy cơ.
