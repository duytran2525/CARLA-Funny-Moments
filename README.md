...
| **`cil_yolo`** | Kết hợp điều khiển bám waypoint CIL và giám sát vật cản RT-DETR-L. | RGB + CIL + RT-DETR-L | RT-DETR-L + GTNet Supervisor (Brake Fusion) |
| **`yolo_detect`** | Chạy chế độ demo hiển thị khả năng phát hiện vật thể thời gian thực của RT-DETR-L. | RT-DETR-L Detector | Traffic Supervisor (Phản động) |
...
*   [traffic_supervisor.py](core_control/traffic_supervisor.py): Bộ giám sát luật lệ giao thông phản động. Phát hiện xe chắn phía trước, đèn giao thông đỏ để can thiệp phanh tức thời (nhận bounding box từ RT-DETR-L).
...
*   [yolo_detector.py](core_perception/yolo_detector.py): Bộ tích hợp nhận diện vật thể RT-DETR-L và tracking BoT-SORT/ByteTrack.
...
# Chạy CIL kết hợp RT-DETR-L + GTNet Supervisor (Bản đầy đủ an toàn nhất)
...
*   **Mô hình Nhận diện Đèn đỏ & Vật cản (RT-DETR-L):** Việc sử dụng TensorRT engine `models/rtdetr-l.engine` là bắt buộc trong môi trường thời gian thực. Nếu sử dụng tệp `.pt` thông thường, FPS nhận dạng sẽ giảm từ ~150 FPS xuống < 20 FPS, gây trễ nghiêm trọng cho bộ phanh khẩn cấp phản động.
