"""
Inverse Perspective Mapping (IPM) – Phase 2

Biến đổi tọa độ điểm đáy bounding box trên ảnh 2D thành tọa độ thực
(x_m, y_m) trên mặt đường (mét), dựa trên giả định mặt phẳng phẳng và
các thông số camera đã biết từ CARLA.

Ứng dụng
--------
* Cung cấp tọa độ (X, Y) mét cho module ADAS để tính TTC chính xác.
* Tạo Pseudo-BEV (Bird's Eye View) cho Phase 4 (visualization).

Nguyên lý toán học
------------------
Dựa trên mô hình camera lỗ kim (pinhole camera model) và giả định mặt phẳng
phẳng (flat-ground assumption):
...
class InversePerspectiveMapping:
    """
    Biến đổi pixel → tọa độ thực trên mặt đường (giả định phẳng).
    ...
    Examples
    --------
    >>> ipm = InversePerspectiveMapping(image_width=1920, image_height=1080)
    >>> x_m, y_m, valid = ipm.pixel_to_world(960, 900)
    ...
    def _setup_params(
        ...
        # Tiêu cự (pixels) từ FOV ngang và kích thước ảnh
        # fx = fy do pixel vuông (square pixels)
